...
<body>
	...
	<div class="cf">
		<div class="fl">
			<a class="btn" href="<?php echo U('Articlezt/cate_add');?>">增加分类</a>
			<button class="btn ajax-post confirm" target-form="ids" href="<?php echo U('Articlezt/cate_del');?>">删除分类</button>
		</div>
	...
	<div class="data-table">
		<table class="">
			...
			<?php if(is_array($cate_content_data)): $i = 0; $__LIST__ = $cate_content_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><span><a href="<?php echo U('Articlezt/indexzt?id='.$vo['id'].'&head='.$vo['title']);?>"><?php echo ($vo["title"]); ?></a> </span></td>
					<td><span><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></span></td>
					<td><span>
                        <?php if($vo['pro_img']): ?><button class="btn pro_img" url="http://120.25.226.231<?php echo ($vo['img']['path']); ?>">有图</button>
							<?php else: ?>
					...
					<td>
						<a href="<?php echo U('Articlezt/cate_edit?status=1&ids='.$vo['id']);?>" class="">编辑</a>

						<a href="<?php echo U('Articlezt/cate_setStatus?status=1&ids='.$vo['id']);?>" class="confirm ajax-get">启动</a>
						<a href="<?php echo U('Articlezt/cate_setStatus?status=0&ids='.$vo['id']);?>" class="confirm ajax-get">禁用</a>
